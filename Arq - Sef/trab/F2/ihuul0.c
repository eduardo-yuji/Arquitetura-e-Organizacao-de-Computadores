...
//if para o tipo i
  ...
  sscanf(aux, "%[^ ] %[^,], %[^,], %[^\n]\n", ass1.opcode, ass1.rd, ass1.rs, ass1.cons);
  if(strcmp("beq",ass.opcode) == 0){
      strcpy(bin.opcode, "1100 ");
    }else if(strcmp("blt",ass.opcode) == 0){
      strcpy(bin.opcode, "1101 ");
    }// if opcode
...
fprintf(arq2, "%s %s %s %d\n", bin1.opcode, bin1.rs, bin1.rd, bin1.cons );

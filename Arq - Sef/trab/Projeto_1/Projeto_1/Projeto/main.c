...
void convertBinary(Operacoes op, FILE *saida) {
	switch(op.operation) {
	    ...
	    case "sub":
		    ...
		    switch(op.register[1]) { //Rs = 1st register operand (5 bits)
		        case "t0":
		    ...
		    switch(op.register[2]) { //Rt = 2nd register operand (5 bits)
		        case "t0":
		    ...
		    switch(op.register[0]) { //Rd = register destination (5 bits)
		        case "t0":
	    ...
	    case "addi":
	        ...
	        switch(op.register[0]) { //Rs = 1st register operand (5 bits)
		        case "t0":
		    ...
		    switch(op.register[1]) { //Rt = 2nd register operand (5 bits)
		        case "t0":
...
void readFile(FILE *entrada, Operacoes op, FILE *saida) {
    char aux[100];
    while(fgest(aux, 100, entrada) != NULL) {
        sscanf(aux, "%[^ ] %[^,], %[^,], %[^\n]\n", op.operation, op.register[0], op.register[1], op.register[2]);
